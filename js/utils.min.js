function renderNewForm(){
    ...
    span.innerText="~/Users/bluehazard:$";
...
const checkCtrlL=e=>{
    ...
    {
        removeAllMainNodes();renderNewForm()
    }
};
const output=el=>{
...
const toggleHTML=currentType=>{
    ...
    const button=document.querySelector("button#change-type");button.innerText=`Change View (${currentType.charAt(0).toUpperCase()+currentType.slice(1)} Website Mode)`
};
const toggleType=currentType=>{
    window.location.pathname="/";
    const newType=currentType==="normal"?"terminal":"normal";
    localStorage.setItem("KAVIN_WEBSITE_TYPE",newType);
    toggleCSS(currentType);toggleHTML(currentType);
    return newType
};
